...
# SEED advantage and SDAR teacher/OPD loss schedule.
...
if [[ "$SEED_ANALYSIS_PROMPT_VERSION" == "strategy_bank" ]]; then
    SEED_ANALYSIS_PROMPT_VERSION=search_strategy_bank
elif [[ "$SEED_ANALYSIS_PROMPT_VERSION" == "skill_only" ]]; then
    SEED_ANALYSIS_PROMPT_VERSION=search_skill_only
fi

# Experiment naming and output location.
PROJECT_NAME=${PROJECT_NAME:-agentic_search}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-seed_qwen2.5_3b_search_both_no_skill_loss_base}
