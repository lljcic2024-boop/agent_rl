...
EZPOINTS_SELF_DIR_SUFFIX="${EZPOINTS_SELF_DIR_SUFFIX:-$SFT_SELF_DIR_SUFFIX}"
DATA_RECORDS="${DATA_RECORDS:-1440}"
DATA_DIR="${DATA_DIR:-$PROJECT_ROOT/outputs/ezpoints_episode_skill_pipeline_qwen25_vl_3b_${EZPOINTS_SELF_DIR_SUFFIX}_${DATA_RECORDS}}"
...
PROJECT_NAME="${PROJECT_NAME:-ezpoints-episode-skill-sft}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-qwen25-vl-3b-ezpoints-episode-skill-sft-${SFT_SELF_SUFFIX}-ep${TOTAL_EPOCHS}}"
OUTPUT_DIR="${OUTPUT_DIR:-$MODELS_ROOT/outputs/sft/ezpoints_episode_skill_sft_${EZPOINTS_SELF_DIR_SUFFIX}_ep${TOTAL_EPOCHS}_${RUN_ID}}"
LOG_DIR="${LOG_DIR:-$MODELS_ROOT/logs/sft}"
LOG_FILE="${LOG_FILE:-$LOG_DIR/ezpoints_episode_skill_sft_${EZPOINTS_SELF_DIR_SUFFIX}_ep${TOTAL_EPOCHS}_${RUN_ID}.log}"
EXPORT_MODEL_DIR="${EXPORT_MODEL_DIR:-$MODELS_ROOT/Qwen2.5-VL-3B-Instruct-ezpoints-episode-skill-sft-${SFT_SELF_SUFFIX}}"
...
if [[ "$MODEL_PATH" == /* && ! -d "$MODEL_PATH" && ! -f "$MODEL_PATH" ]]; then
    ...
    echo "Set MODEL_PATH to a local Qwen2.5-VL checkpoint or a resolvable Hugging Face model id." >&2
    exit 1
fi
...
if [[ "$EXPORT_MODEL_AFTER_TRAIN" == "true" ]]; then
    ...
    echo "Exported EZPoints episode-skill SFT model to $EXPORT_MODEL_DIR"
fi
